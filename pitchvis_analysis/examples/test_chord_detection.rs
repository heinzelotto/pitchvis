...
fn main() {
    ...
    let chords = vec![
        (
            "C Minor",
            Chord::new(PitchClass::C, ChordQuality::Minor, ChordNumber::Triad),
        ),
        (
            "C Dominant 7th",
            Chord::new(PitchClass::C, ChordQuality::Dominant, ChordNumber::Seventh),
        ),
        (
            "C Major 7th",
            Chord::new(PitchClass::C, ChordQuality::Major, ChordNumber::Seventh),
        ),
        (
            "C Minor 7th",
            Chord::new(PitchClass::C, ChordQuality::Minor, ChordNumber::Seventh),
        ),
    ];
    ...
    let test_chords = vec![
        (
            "C Major",
            Chord::new(PitchClass::C, ChordQuality::Major, ChordNumber::Triad),
        ),
        (
            "C Minor",
            Chord::new(PitchClass::C, ChordQuality::Minor, ChordNumber::Triad),
        ...
        (
            "E Minor",
            Chord::new(PitchClass::E, ChordQuality::Minor, ChordNumber::Triad),
        ),
    ];

    for (name, chord) in &test_chords {
        let chord_notes = chord.notes();
        let chord_pitch_classes: Vec<PitchClass> =
            chord_notes.iter().map(|n| n.pitch_class.clone()).collect();
        println!("    {} has pitch classes: {:?}", name, chord_pitch_classes);

        // Check if all detected notes are in this chord
        let all_match = detected_pitch_classes
            .iter()
